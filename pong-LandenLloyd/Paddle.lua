...
function Paddle:goToTarget(target)
    --Moves the paddle so that the target is somewhere within the paddle
    if self.y > target then
        self.dy = -AI_PADDLE_SPEED
    elseif self.y + self.height < target then
        self.dy = AI_PADDLE_SPEED
    else
        self.dy = 0
    end
